    ...
    : IHostedLifecycleService
{
        ...
        CancellationToken cancellationToken)
    {
                ...
                await missionControlClient.TryPublishAsync(
                    eventType:
                        HappyDiscardEventTypes
                            .ServiceStarted,
                    payload:
                        new DiscardServiceStartedEvent(
                            $"{listenAddress}:{options.Value.Port}"),
                    payloadTypeInfo:
                        HappyDiscardJsonContext
                            .Default
                            .DiscardServiceStartedEvent,
                    occurredAt:
                        occurredAt,
                    correlationId:
                        null,
        ...
        {
            logger.LogWarning(
                "Mission Control publication for Discard Service Started timed out.");
        }
